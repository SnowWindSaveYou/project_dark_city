...
游戏剧情视觉层现有架构仅有 NanoVG 文字与对话 UI，**DialogueSystem 目前不支持背景图像显示**。本节记录规划中的 CG 集成方案，供后续实现参考。

**关联文档**：`docs/cg_asset_brief.md`（完整素材需求与设计说明）、`docs/image-generation-style-guide.md`（图像生成风格指南）

### 8.5.2 CG 资产规划

按照 `cg_asset_brief.md`，全局约 51 张插画，分三类：

| 类型 | 数量 | 规格 | 演出方式 |
|------|------|------|------|
| A类：背景底图 | 8 | 16:9，1024×576 | 对话全程持续，低调垫底 |
| B类：过渡切换帧 | 15 | 16:9，1024×576 | 快速切换约0.5s，无对话框 |
| C类：关键情绪帧 | 28 | 16:9，1024×576 | 停留至玩家点击，可配对话框 |

> **演出方案**：物语分镜式背景替换。游戏横屏，所有CG统一16:9，同层切换即分镜。竖版/特写感靠**构图**实现（主体居中留黑边、画面内叠黑条），不依赖文件尺寸。

**已完成图像（截至2026-05-15）**：
- BG-02（公寓客厅·深夜）：`assets/image/bg_apartment_living_night_v5_20260515071406.png` ✅ 已验证风格基准
- 棠立绘参考：`assets/image/棠_立绘_刀_v1_20260515070049.png`、`棠_立绘_v4_20260515065722.png` ✅

**当前阶段**：优先生成 A类背景底图 8 张，用于 DialogueSystem 背景图支持上线后的第一批接入测试。

### 8.5.3 DialogueSystem 背景图支持（待实现）

**需要修改的模块**：`DialogueSystem.lua`
...
-- 对话数据新增可选 bgImage 字段
{
    speaker = "苏柚",
    text    = "...",
    bgImage = "image/bg_apartment_living_night_v5.png",  -- 可选
}

-- DialogueSystem 需要：
-- 1. 在 draw() 中，若当前帧有 bgImage，先绘制背景图
-- 2. bgImage 覆盖整个对话面板底部（全屏拉伸或裁剪填充）
-- 3. bgImage 持续到下一个有 bgImage 的帧，或对话结束
-- 4. 关键情绪帧（keyframe）可覆盖整个屏幕，压过对话框
```

**过渡切换帧集成方式**：
- 过渡帧在台词之间以"插页"方式显示，持续约 0.5-1 秒后自动推进
- 通过对话数据中新增 `transitionImage` 字段触发

**实现优先级**：先实现 A类背景底图支持（最简单：只需在对话底部渲染一张16:9图），再实现C类情绪帧全屏叠加，最后实现B类过渡帧插页。

### 8.5.4 色调系统与叙事规则

**强制规则**（代码层面也要遵守）：

| 视觉环境 | 色调 | 对应 bgImage 前缀 |
|---------|------|-----------------|
| 现实·日常 | 暖黄/橙色调 | `bg_apartment_*` |
| 暗面 | 灰蓝/深紫，去饱和 | `bg_dark_world_*` |
| 记忆·棠 | **过曝白，刻意焦白** | `bg_memory_*` |

棠的记忆画面（`bg_memory_overexposed`）贯穿第2天到第13天的所有棠相关闪回，**绝不能混用现实或暗面色调**。

### 8.5.5 制作与接入流程

```
第一步: 生成 A类背景底图 8 张（当前阶段）
  ↓
第二步: DialogueSystem 实现 bgImage 基础支持
  ↓
第三步: 接入测试，用 BG-02 验证渲染效果
  ↓
第四步: 生成关键情绪帧（第二批：11张主线核心帧）
  ↓
第五步: 实现 C类情绪帧全屏叠加逻辑
  ↓
第六步: 生成 B类过渡切换帧 + 实现插页逻辑
  ↓
第七步: 生成 D类结局帧 + 接入结局演出
```
