...
##
## 视觉设计：
##   - 纯白背景 + MonogatariBG 几何层（由 main.tscn 提供 CanvasLayer 2）
##   - 左侧：巨大偏移标题 / 竖切线 / 坐标小字 / 4个极简按钮
##   - 右侧：白夜立绘（半透明叠加）
##   - 入场：瞬切 + 位移（非淡入）
...
## 白夜立绘候选（优先第一张，不存在时尝试第二张）
const BAIYE_PATHS: Array = [
	"res://assets/image/白夜_chibi_20260506003802.png",
	"res://assets/image/白夜_chibi_20260509133337.png",
]

# 物语系列配色（针对白底）
const C_TITLE:    Color = Color(0.04, 0.03, 0.08, 1.0)   # 近黑
const C_SUBTITLE: Color = Color(0.28, 0.25, 0.38, 0.85)   # 深紫灰
const C_COORD:    Color = Color(0.50, 0.46, 0.60, 0.60)   # 淡紫
const C_VERSION:  Color = Color(0.55, 0.52, 0.62, 0.45)
const C_LINE:     Color = Color(0.18, 0.16, 0.28, 0.35)   # 竖切线

# 按钮配色
const C_BTN_START_BG:    Color = Color(0.08, 0.06, 0.14, 1.0)
const C_BTN_START_HOVER: Color = Color(0.18, 0.12, 0.32, 1.0)
const C_BTN_SEC_BG:      Color = Color(0.0,  0.0,  0.0,  0.0)   # 透明底
...
@onready var _title_large: Label     = $TitleBlock/TitleLarge
...
func _ready() -> void:
	# 背景色：纯白，让 MonogatariBG CanvasLayer 透出
	_set_white_bg()

	# 标题样式
	_title_large.add_theme_color_override("font_color", C_TITLE)
	...
	# 初始隐藏（入场动画起始位置）
	_title_large.modulate.a = 0.0
	_title_large.position.x -= 40.0
...
func _style_btn_primary(btn: Button) -> void:
	...
	n.corner_radius_top_left    = 0
	n.corner_radius_top_right   = 0
	n.corner_radius_bottom_left = 0
	n.corner_radius_bottom_right = 0
	...
	h.bg_color = C_BTN_START_HOVER
	btn.add_theme_stylebox_override("hover", h)

	var p: StyleBoxFlat = n.duplicate()
	p.bg_color = Color(0.28, 0.18, 0.48, 1.0)
...
func _style_btn_secondary(btn: Button) -> void:
	...
	var p: StyleBoxFlat = n.duplicate()
	p.bg_color = Color(0.18, 0.14, 0.28, 0.12)
	btn.add_theme_stylebox_override("pressed", p)


func _style_btn_quit(btn: Button) -> void:
	_style_btn_secondary(btn)
	var n: StyleBoxFlat = btn.get_theme_stylebox("normal").duplicate()
...
func _draw_decorations() -> void:
	...
	# 竖切线（左侧标题区右边缘附近）
	var lx: float = w * 0.59
	_draw_layer.draw_line(Vector2(lx, h * 0.10), Vector2(lx, h * 0.90),
		C_LINE, 1.0)

	# 横细线（标题下方）
	var title_y: float = h * 0.36
	_draw_layer.draw_line(Vector2(72.0, title_y), Vector2(w * 0.54, title_y),
	...
	if font:
		_draw_layer.draw_string(font, Vector2(72.0, h * 0.14),
			...
			-1, 11, Color(C_COORD.r, C_COORD.g, C_COORD.b, 0.5))
...
func _play_enter_anim() -> void:
	var BASE_OFFSET: float = 40.0

	# 0. 立绘滑入（从右 +30 → 0）
	var t0: Tween = create_tween()
	t0.set_parallel(true)
	t0.tween_property(_baiye_sprite, "modulate:a", 1.0, 0.35) \
		.set_ease(Tween.EASE_OUT).set_trans(Tween.TRANS_QUAD)
	t0.tween_property(_baiye_sprite, "position:x",
		_baiye_sprite.position.x - 30.0, 0.35) \
		.set_ease(Tween.EASE_OUT).set_trans(Tween.TRANS_QUAD)

	# 1. 大标题：先保持隐藏，0.05s 后瞬间显现 + 快速归位
	var t1: Tween = create_tween()
	t1.tween_interval(0.05)
	t1.set_parallel(false)
	t1.tween_callback(func() -> void:
		_title_large.modulate.a = 1.0
	)
	t1.set_parallel(true)
	t1.tween_property(_title_large, "position:x",
		_title_large.position.x + BASE_OFFSET, 0.22) \
...
func _build_gallery_overlay() -> Control:
	...
	cb_n.bg_color = Color(0.0, 0.0, 0.0, 0.0)
	cb_n.border_color = Color(0.18, 0.16, 0.28, 0.45)
	cb_n.set_border_width_all(1)
	cb_n.corner_radius_top_left    = 0
	cb_n.corner_radius_top_right   = 0
	cb_n.corner_radius_bottom_left = 0
	cb_n.corner_radius_bottom_right = 0
	close_btn.add_theme_stylebox_override("normal", cb_n)
	var cb_h: StyleBoxFlat = cb_n.duplicate()
	cb_h.bg_color = Color(0.10, 0.08, 0.16, 0.08)
...
func _build_ending_card(entry: Dictionary) -> PanelContainer:
	...
	if unlocked:
		card_style.bg_color     = Color(0.94, 0.93, 0.97, 1.0) if is_vic else Color(0.97, 0.93, 0.93, 1.0)
		card_style.border_color = Color(0.38, 0.30, 0.55, 0.45) if is_vic else Color(0.65, 0.20, 0.20, 0.45)
	else:
		card_style.bg_color     = Color(0.92, 0.92, 0.93, 1.0)
		card_style.border_color = Color(0.55, 0.53, 0.58, 0.30)
	...
	icon_lbl.add_theme_color_override("font_color",
		...
		Color(0.65, 0.20, 0.20) if (unlocked and not is_vic) else
	...
	title_lbl.add_theme_color_override("font_color",
		C_TITLE if unlocked else Color(0.55, 0.53, 0.58))
	title_lbl.size_flags_horizontal = Control.SIZE_EXPAND_FILL
	...
	sub_lbl.add_theme_color_override("font_color",
		C_COORD if unlocked else Color(0.60, 0.58, 0.62))
...
func _on_start_pressed() -> void:
	...
	tw.tween_property(self, "modulate:a", 0.0, 0.3) \
		.set_ease(Tween.EASE_IN).set_trans(Tween.TRANS_QUAD)
	tw.tween_callback(func() -> void: pass)
