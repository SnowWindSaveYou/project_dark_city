...
# 裂隙弹窗
const RIFT_POPUP_W := 260
const RIFT_POPUP_H := 185
const RIFT_BTN_W := 100
const RIFT_BTN_H := 30
const RIFT_BTN_GAP := 12

# Toast
const TOAST_W := 220
const TOAST_R := 10
const TOAST_GAP := 8
const TOAST_MARGIN_R := 12
const TOAST_MARGIN_T := 52
const TOAST_MAX := 3
const TOAST_IDLE_TIME := 3.0
const TOAST_ENTER_TIME := 0.35
const TOAST_EXIT_TIME := 0.25

# 相片拍立得尺寸
const PHOTO_W := 200
const PHOTO_H := 260
const PHOTO_BORDER := 10
const PHOTO_BOTTOM := 40
const PHOTO_R := 3
...
var _photo_card_type := ""
var _photo_location := ""
var _photo_rotation := 0.0  # 拍立得微倾角度
...
var _effects_t := 0.0
var _button_t := 0.0
var _btn_hover_t := 0.0

# ===========================================================================
# 裂隙确认弹窗状态
# ===========================================================================
var _rift_active := false
var _rift_phase := "none"
var _rift_cx := 0.0
var _rift_cy := 0.0
var _rift_overlay_alpha := 0.0
var _rift_popup_scale := 0.0
var _rift_popup_alpha := 0.0
var _rift_icon_t := 0.0
var _rift_title_t := 0.0
var _rift_desc_t := 0.0
var _rift_btn_enter_t := 0.0
var _rift_btn_stay_t := 0.0
var _rift_btn_enter_hover := 0.0
var _rift_btn_stay_hover := 0.0
...
func show_toast(card_type: String, applied_effects: Dictionary = {},
	...
	var tid := _toast_next_id
	_toast_next_id += 1

	# 文案
	var tmpl := _pick_template(card_type, trap_subtype)
...
func _handle_rift_click(pos: Vector2) -> void:
	if _rift_phase == "enter":
		return
	var hit := _rift_btn_hit_test(pos)
	if hit == "enter":
		dismiss_rift(true)
	elif hit == "stay":
		dismiss_rift(false)
		...
		# 面板外点击 = 留下
		var px := _rift_cx - RIFT_POPUP_W / 2.0
		var py := _rift_cy - RIFT_POPUP_H / 2.0
...
func _handle_toast_click(pos: Vector2) -> bool:
	...
	for i in range(_toast_queue.size() - 1, -1, -1):
		...
		if t["phase"] == "done" or t["phase"] == "exit":
			continue
		var rect := Rect2(t["draw_x"], t["draw_y"], t["draw_w"], t["draw_h"])
...
func _rift_btn_hit_test(pos: Vector2) -> String:
	var btn_y := _rift_cy + RIFT_POPUP_H / 2.0 - RIFT_BTN_H - 14.0
	var total_w := RIFT_BTN_W * 2.0 + RIFT_BTN_GAP
	var start_x := _rift_cx - total_w / 2.0
	# 进入按钮
	if Rect2(start_x, btn_y, RIFT_BTN_W, RIFT_BTN_H).has_point(pos):
		return "enter"
	# 留下按钮
	var stay_x := start_x + RIFT_BTN_W + RIFT_BTN_GAP
...
func _draw_event() -> void:
	...
	# 面板尺寸
	var popup_w := vp.x * 0.7
	var popup_h := vp.y * 0.55
	var cx := vp.x / 2.0
	var cy := vp.y / 2.0
	var scaled_w := popup_w * _panel_scale
	var scaled_h := popup_h * _panel_scale
	var panel_x := cx - scaled_w / 2.0
	var panel_y := cy - scaled_h / 2.0
	...
	var content_alpha := _panel_alpha

	# 图标
	if _icon_t > 0.01:
		var icon_color := Color(t.text_primary.r, t.text_primary.g, t.text_primary.b, content_alpha * _icon_t)
	...
	# 标题
	if _title_t > 0.01:
		var title_color := Color(type_color.r, type_color.g, type_color.b, content_alpha * _title_t)
	...
	# 描述
	if _desc_t > 0.01:
		var desc_color := Color(t.text_secondary.r, t.text_secondary.g, t.text_secondary.b, content_alpha * _desc_t)
	...
	if _effects_t > 0.01:
		...
		for key in effects:
			...
			var effect_text := icon_str + " " + prefix + str(delta_val)
	...
	# 关闭提示
	if _button_t > 0.01:
		var btn_color := Color(t.text_secondary.r, t.text_secondary.g, t.text_secondary.b, _button_t * 0.6)
...
func _draw_photo() -> void:
	...
	# 相片白底 (略带暖白)
	var paper_color := Color(0.988, 0.980, 0.961, _panel_alpha * 0.98)
	draw_rect(Rect2(photo_x, photo_y, PHOTO_W, PHOTO_H), paper_color)

	# 相片边框
	var edge_color := Color(0.824, 0.784, 0.725, _panel_alpha * 0.47)
	draw_rect(Rect2(photo_x, photo_y, PHOTO_W, PHOTO_H), edge_color, false, 0.8)

	# 内部照片区
	var img_x := photo_x + PHOTO_BORDER
	var img_y := photo_y + PHOTO_BORDER
	var img_w := PHOTO_W - PHOTO_BORDER * 2.0
	var img_h := PHOTO_H - PHOTO_BORDER - PHOTO_BOTTOM

	# 照片底色 (暗蓝灰)
	draw_rect(Rect2(img_x, img_y, img_w, img_h), Color(0.098, 0.118, 0.157, _panel_alpha * 0.94))

	# 照片内氛围色 (事件类型颜色)
	var atmos_color := Color(type_color.r, type_color.g, type_color.b, _panel_alpha * 0.15)
	draw_rect(Rect2(img_x, img_y, img_w, img_h), atmos_color)

	# 事件图标
	if _icon_t > 0.01:
		var icon_color := Color(1, 1, 1, _panel_alpha * _icon_t * 0.9)
	...
	# 事件标题
	if _title_t > 0.01:
		var title_color := Color(type_color.r, type_color.g, type_color.b, _panel_alpha * _title_t * 0.9)
	...
	# 描述
	if _desc_t > 0.01:
		var desc_color := Color(0.784, 0.784, 0.824, _panel_alpha * _desc_t * 0.7)
		var desc := _card.get_event_text()
	...
	var tape_w := 36.0
	var tape_h := 10.0
	var tape_x := cx - tape_w / 2.0
	var tape_y := photo_y - tape_h / 2.0
	var tape_color := Color(0.863, 0.824, 0.706, _panel_alpha * 0.63)
	...
	# 关闭提示
	if _button_t > 0.01:
		var hint_color := Color(0.706, 0.667, 0.608, _panel_alpha * _button_t * 0.7)
...
func _draw_rift_confirm() -> void:
	...
	var hw := RIFT_POPUP_W / 2.0
	var hh := RIFT_POPUP_H / 2.0
	var px := _rift_cx - hw
	var py := _rift_cy - hh

	# 面板背景
	var panel_color := Color(t.panel_bg.r, t.panel_bg.g, t.panel_bg.b, _rift_popup_alpha * 0.96)
	draw_rect(Rect2(px, py, RIFT_POPUP_W, RIFT_POPUP_H), panel_color)

	# 面板边框 (暗面风格)
	var border_color := Color(t.dark_accent.r, t.dark_accent.g, t.dark_accent.b, _rift_popup_alpha * 0.4)
	draw_rect(Rect2(px, py, RIFT_POPUP_W, RIFT_POPUP_H), border_color, false, 1.2)

	# 顶部色条
	var accent_color := Color(t.dark_accent.r, t.dark_accent.g, t.dark_accent.b, _rift_popup_alpha)
	draw_rect(Rect2(px, py, RIFT_POPUP_W, 4), accent_color)

	# 图标
	if _rift_icon_t > 0.01:
		var icon_color := Color(t.dark_accent.r, t.dark_accent.g, t.dark_accent.b, _rift_popup_alpha * _rift_icon_t)
	...
	if _rift_title_t > 0.01:
		var title_color := Color(t.text_primary.r, t.text_primary.g, t.text_primary.b, _rift_popup_alpha * _rift_title_t)
	...
	if _rift_desc_t > 0.01:
		var desc_color := Color(t.text_secondary.r, t.text_secondary.g, t.text_secondary.b, _rift_popup_alpha * _rift_desc_t * 0.7)
	...
	# 双按钮
	var btn_y := _rift_cy + hh - RIFT_BTN_H - 14.0
	var total_w := RIFT_BTN_W * 2.0 + RIFT_BTN_GAP
	var start_x := _rift_cx - total_w / 2.0
	...
	if _rift_btn_enter_t > 0.01:
		...
		var btn_text_color := Color(1, 1, 1, enter_alpha * 0.94)
	...
	if _rift_btn_stay_t > 0.01:
		var bx := start_x + RIFT_BTN_W + RIFT_BTN_GAP
		var stay_alpha := _rift_popup_alpha * _rift_btn_stay_t
		var stay_bg := Color(t.text_secondary.r, t.text_secondary.g, t.text_secondary.b, stay_alpha * 0.24)
		draw_rect(Rect2(bx, btn_y + (1.0 - _rift_btn_stay_t) * 15, RIFT_BTN_W, RIFT_BTN_H), stay_bg)
		var stay_border := Color(t.text_secondary.r, t.text_secondary.g, t.text_secondary.b, stay_alpha * 0.47)
		draw_rect(Rect2(bx, btn_y + (1.0 - _rift_btn_stay_t) * 15, RIFT_BTN_W, RIFT_BTN_H), stay_border, false, 1.0)
		var stay_text_color := Color(t.text_primary.r, t.text_primary.g, t.text_primary.b, stay_alpha * 0.78)
...
func _draw_toasts() -> void:
	...
	for toast in _toast_queue:
		...
		var item_h := _toast_item_h(toast)
		...
		# 背景
		var bg_color := Color(t.panel_bg.r, t.panel_bg.g, t.panel_bg.b, alpha * 0.94)
		draw_rect(Rect2(x, y, TOAST_W, item_h), bg_color)

		# 边框
		var border_color := Color(t.panel_border.r, t.panel_border.g, t.panel_border.b, alpha * 0.31)
		...
		# 内容区域
		var content_x := x + 12.0
		var content_y := y + 14.0
		...
		var title_color := Color(type_color.r, type_color.g, type_color.b, alpha * 0.94)
		...
		var desc_color := Color(t.text_secondary.r, t.text_secondary.g, t.text_secondary.b, alpha * 0.78)
		...
		if toast["phase"] == "idle":
			...
			# 背景
			var bar_bg := Color(t.text_secondary.r, t.text_secondary.g, t.text_secondary.b, alpha * 0.12)
			draw_rect(Rect2(x + 10, bar_y, bar_w, 2), bar_bg)

			# 前景
			var bar_fg := Color(type_color.r, type_color.g, type_color.b, alpha * 0.47)
