...
var _flash_dirs := { "san": 0, "order": 0, "money": 0 }  # +1 增 / -1 减
...
var _dark_layer_name := ""
var _dark_energy := 0
var _dark_max_energy := 10
var _dark_layer_idx := 1
var _dark_layer_count := 3
var _dark_energy_flash := 0.0

# 退出按钮几何缓存 (用于 hit test)
var _dark_exit_rect := Rect2()
...
func _draw() -> void:
	...
	var strip_x := (w - strip_w) / 2.0
	var strip_y := float(MARGIN_TOP)
	var strip_cy := strip_y + STRIP_H / 2.0
	...
	# === 右半区域 ===
	var right_x := strip_x + strip_w - PAD_X - 6.0
	if _dark_mode:
...
func _draw_normal_strip(sx: float, sy: float, sw: float, _scy: float,
		_font: Font, t) -> void:
	# 纸张阴影
	var shadow_rect := Rect2(sx - 4, sy + 1, sw + 8, STRIP_H + 6)
	...
	# 横线纹理 (淡蓝色)
	var line_y := sy + LINE_SPACING * 0.6
	...
	# 三边边框 (底部留给撕裂)
	var border_color := Color(t.notebook_border, 0.4)
...
func _draw_dark_strip(sx: float, sy: float, sw: float, _scy: float,
		_font: Font, _t) -> void:
	# 深邃阴影
	var shadow_rect := Rect2(sx - 6, sy - 2, sw + 12, STRIP_H + 10)
	draw_rect(shadow_rect, Color(0.03, 0.015, 0.08, 0.35))

	# 面板主体 (深紫渐变)
	# Godot _draw 没有渐变 rect，用两段模拟
	var top_color := Color(0.118, 0.094, 0.196, 0.88)  # (30,24,50)/255
	var bot_color := Color(0.071, 0.055, 0.137, 0.94)   # (18,14,35)/255
	draw_rect(Rect2(sx, sy, sw, STRIP_H / 2), top_color)
	draw_rect(Rect2(sx, sy + STRIP_H / 2, sw, STRIP_H / 2), bot_color)

	# 横纹灵纹 (暗紫, 对应笔记本横线)
	var line_y := sy + LINE_SPACING * 0.6
	var vein_color := Color(0.545, 0.361, 0.965, 0.06)  # #8B5CF6 alpha 16/255
	while line_y < sy + STRIP_H:
		draw_line(Vector2(sx + 4, line_y), Vector2(sx + sw - 4, line_y),
			vein_color, 0.4)
		line_y += LINE_SPACING
...
func _draw_resources(sx: float, sy: float, _sw: float,
	...
	for i in count:
		...
		var display_num := int(round(_display_values[key]))
		...
		var icon_w := font.get_string_size(icon_str, HORIZONTAL_ALIGNMENT_LEFT, -1, 15).x
		var icon_end := cx + icon_w
		...
		var label_w := font.get_string_size(label_str, HORIZONTAL_ALIGNMENT_LEFT, -1, 9).x

		# --- 数值 ---
		var val_str := str(display_num)
		var val_color: Color = rc
		var flash_t: float = _flash_timers.get(key, 0.0)
		if flash_t > 0:
			var pulse := 0.5 + 0.5 * sin(flash_t * 20.0)
		...
		var num_w := font.get_string_size(val_str, HORIZONTAL_ALIGNMENT_LEFT, -1, 15).x

		# --- 分隔竖线 ---
		var next_x := icon_end + 1 + maxf(num_w, label_w) + 10.0
		if i < count - 1:
			var sep_color: Color
			if _dark_mode:
...
func _draw_normal_right(sy: float, scy: float, right_x: float,
	...
	var weather_type := Weather.get_weather(day)
	var weather_icon := Weather.get_icon(weather_type)
	var weather_name := Weather.get_name(weather_type)
	...
	var day_text_w := font.get_string_size(day_text, HORIZONTAL_ALIGNMENT_LEFT, -1, 14).x
	var day_right := right_x - 36.0
	draw_string(font, Vector2(day_right - day_text_w, scy - 2), day_text,
		HORIZONTAL_ALIGNMENT_LEFT, -1, 14, Color(t.text_primary, 0.86))

	# "第X天" (副文字)
	var sub_text := "第" + str(day) + "天"
	var sub_w := font.get_string_size(sub_text, HORIZONTAL_ALIGNMENT_LEFT, -1, 9).x
	draw_string(font, Vector2(day_right - sub_w, scy + 9), sub_text,
		HORIZONTAL_ALIGNMENT_LEFT, -1, 9, Color(t.text_secondary, 0.55))

	# 分隔竖线
	var sep_x := day_right - maxf(day_text_w, sub_w) - 8.0
...
func _draw_dark_right(sx: float, sy: float, _sw: float, scy: float,
	...
	# --- 层级信息组 ---
	var info_right_x := icon_cx - icon_r * 2 - 8.0

	# 层级名称 (主文字)
	var layer_name := _dark_layer_name if _dark_layer_name != "" else "暗面"
	var layer_w := font.get_string_size(layer_name, HORIZONTAL_ALIGNMENT_LEFT, -1, 14).x
	...
	# 能量副行
	var energy_ratio := float(_dark_energy) / maxf(float(_dark_max_energy), 1.0)
	var energy_text := "⚡" + str(_dark_energy) + "/" + str(_dark_max_energy)
	...
	# 闪光
	var flash_alpha := 0.63
	if _dark_energy_flash > 0:
		flash_alpha = 0.5 + 0.5 * sin(_dark_energy_flash * 20.0)
	e_color.a = flash_alpha

	# 能量文字
	var energy_text_w := font.get_string_size(energy_text, HORIZONTAL_ALIGNMENT_LEFT, -1, 9).x
	...
	var mini_bar_w := 30.0
	var mini_bar_h := 4.0
	var m_bar_x := info_right_x - energy_text_w - mini_bar_w - 3.0
	var m_bar_y := scy + 5 - mini_bar_h / 2.0
	...
	# 填充
	var fill_w := mini_bar_w * energy_ratio
	if fill_w > 0:
		draw_rect(Rect2(m_bar_x, m_bar_y, fill_w, mini_bar_h), e_color)

	# --- 分隔线 (层级组和资源区之间) ---
	var sep_x := info_right_x - maxf(layer_w, energy_text_w + mini_bar_w + 3) - 8.0
	draw_line(Vector2(sep_x, sy + SEP_PAD),
		Vector2(sep_x, sy + STRIP_H - SEP_PAD),
...
func _draw_torn_edge(x: float, y: float, w: float, base_color: Color) -> void:
	var tear_color := Color(base_color, 0.6)
	var step := 5.0
	var cx := x
	var i := 0
	while cx < x + w:
		var next_x := minf(cx + step, x + w)
		var dy: float = 1.5 if (i % 2 == 0) else -0.5
		var jitter := sin(cx * 1.7) * 0.8
		var rect_h := 2.0 + absf(dy + jitter)
...
func _draw_dark_edge(x: float, y: float, w: float) -> void:
	var edge_color := Color(0.071, 0.055, 0.137, 0.94)
	var step := 4.0
	var cx := x
	var i := 0
	while cx < x + w:
		var _next_x := minf(cx + step, x + w)
		var dy: float = 2.0 if (i % 2 == 0) else -0.3
		var jitter := sin(cx * 2.1 + 0.7) * 1.2
		var rect_h := 2.0 + absf(dy + jitter)
...
func _draw_tape(center: Vector2, tw: float, th: float, angle: float) -> void:
	# 半透明胶带主体
	var tape_color := Color(0.961, 0.922, 0.784, 0.24)
	var tape_border := Color(0.863, 0.824, 0.706, 0.16)
...
func _draw_dark_seal(center: Vector2, tw: float, th: float, angle: float) -> void:
	var seal_body := Color(0.545, 0.361, 0.965, 0.10)
	var seal_line := Color(0.133, 0.827, 0.933, 0.14)
	var seal_border := Color(0.545, 0.361, 0.965, 0.14)
