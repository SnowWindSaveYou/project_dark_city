...
const BTN_H := 28.0
const BTN_R := 7.0
const REFRESH_BTN_W := 100.0
const LEAVE_BTN_W := 60.0
const BTN_GAP := 14.0
...
# ---------------------------------------------------------------------------
var _active := false
var _phase := "none"  # "enter"|"idle"|"exit"|"none"
var _goods: Array = []
...
# Hover
var _hover_card := -1
var _hover_refresh := false
var _hover_leave := false
var _hover_card_t: Array = []
var _hover_refresh_t := 0.0
var _hover_leave_t := 0.0

# 刷新动画
var _refresh_phase := "idle"
...
func open_shop() -> void:
	...
	# 按钮
	var btn_delay := 0.12 + 0.07 * (CARD_COUNT + 3)
...
func close_shop() -> void:
	...
	tw.set_parallel(true)
	for i in range(_card_alphas.size()):
		var spin := [deg_to_rad(-15.0), 0.0, deg_to_rad(15.0)]
...
func _refresh_goods() -> void:
	...
	for _i in range(_goods.size()):
		...
		var rand_rot := (randf() - 0.5) * 2.0 * deg_to_rad(8.0)
	...
	if event is InputEventMouseButton:
		...
		if _refresh_phase == "idle":
			for i in range(_goods.size()):
				var card_rect := _get_card_rect(i)
		...
		if _refresh_t > 0.5 and _refresh_phase == "idle":
			var refresh_rect := _get_refresh_button_rect()
		...
		# 检查离开按钮
		if _leave_t > 0.5:
			var leave_rect := _get_leave_button_rect()
...
func _update_hover(pos: Vector2) -> void:
	...
	# 刷新按钮
	if _refresh_t > 0.5:
		var can_afford := GameData.get_resource("money") >= ShopData.REFRESH_COST
		if can_afford:
			var refresh_rect := _get_refresh_button_rect()
...
func _do_refresh() -> void:
	...
	for i in range(CARD_COUNT):
		...
		var target_rot := spin_dir * deg_to_rad(10.0 + randf() * 10.0)
	...
	tw.chain().tween_callback(func():
		...
		for j in range(_card_alphas.size()):
			var rand_rot := (1.0 if randf() > 0.5 else -1.0) * deg_to_rad(4.0 + randf() * 4.0)
...
func _calc_layout(vp: Vector2) -> void:
	...
	var px := (vp.x - _popup_w) / 2.0
	var py := (vp.y - _popup_h) / 2.0
...
func _get_card_rect(index: int) -> Rect2:
	...
	var start_x := _popup_rect.position.x + (_popup_w - total_w) / 2.0
	var x := start_x + index * (CARD_W + CARD_GAP)
	var y := _popup_rect.position.y + CARDS_Y
	return Rect2(x, y, CARD_W, CARD_H)

func _get_refresh_button_rect() -> Rect2:
	var total_btn_w := REFRESH_BTN_W + BTN_GAP + LEAVE_BTN_W
	var x := _popup_rect.position.x + (_popup_w - total_btn_w) / 2.0
	var y := _popup_rect.position.y + _btn_y
	return Rect2(x, y, REFRESH_BTN_W, BTN_H)

func _get_leave_button_rect() -> Rect2:
	var total_btn_w := REFRESH_BTN_W + BTN_GAP + LEAVE_BTN_W
	var x := _popup_rect.position.x + (_popup_w - total_btn_w) / 2.0 + REFRESH_BTN_W + BTN_GAP
	var y := _popup_rect.position.y + _btn_y
...
func _process(delta: float) -> void:
	if not _active:
		return
	# Hover 插值
	var speed := minf(1.0, delta * 12.0)
	for i in range(_hover_card_t.size()):
		var target := 1.0 if _hover_card == i else 0.0
		_hover_card_t[i] += (target - _hover_card_t[i]) * speed
	var r_target := 1.0 if _hover_refresh else 0.0
	_hover_refresh_t += (r_target - _hover_refresh_t) * speed
	var l_target := 1.0 if _hover_leave else 0.0
...
func _draw() -> void:
	...
	# === 面板 ===
	var cx := _popup_rect.get_center().x
	var cy := _popup_rect.get_center().y
	var hw := _popup_w / 2.0
	var hh := _popup_h / 2.0
	...
	# 面板阴影
	var shadow_rect := Rect2(_popup_rect.position - Vector2(8, 4), _popup_rect.size + Vector2(16, 12))
	...
	var bar_rect := Rect2(_popup_rect.position.x + 4, _popup_rect.position.y + 4, _popup_w - 8, 5)
	draw_rect(bar_rect, Color(shop_color.r, shop_color.g, shop_color.b, 0.78))

	var panel_left := _popup_rect.position.x
	var panel_top := _popup_rect.position.y

	# === Header ===
	if _header_t > 0.01:
		var ha := _panel_alpha * _header_t
		# 购物车图标
	...
	# === 描述 ===
	if _desc_t > 0.01:
		var da := _panel_alpha * _desc_t
		var desc_offset := (1.0 - _desc_t) * 6.0
...
func _draw_item_card(index: int, card_t: float, game_time: float) -> void:
	...
	# 呼吸浮动
	var float_y := sin(game_time * 2.0 + index * 1.3) * 1.5
	# Hover 上浮
	var hover_lift := hover_t * 4.0

	var card_cx := card_rect.get_center().x + shake_x
	var card_cy := card_rect.get_center().y + float_y - hover_lift
	var s := card_t * (1.0 + hover_t * 0.04)

	# 保存当前变换，在卡牌中心进行旋转和缩放
	var card_xf := Transform2D()
	...
	# 需要和面板缩放组合
	var panel_cx := _popup_rect.get_center().x
	var panel_cy := _popup_rect.get_center().y
	var panel_xf := Transform2D()
	...
	var base_alpha := _panel_alpha * minf(card_t * 1.5, 1.0)
	var draw_alpha := base_alpha * (0.3 if sold else 1.0)

	var hw := CARD_W / 2.0
	var hh := CARD_H / 2.0
	var local_rect := Rect2(-hw, -hh, CARD_W, CARD_H)

	# ── 卡牌阴影 ──
	var shadow_r := Rect2(-hw + 1, -hh + 3, CARD_W + 2, CARD_H + 2)
	draw_rect(shadow_r, Color(0, 0, 0, 0.12 * draw_alpha))

	# ── 卡牌背景 ──
	var bg_a := 0.82 if (sold or not can_afford) else 0.96
	...
	if sold:
		...
		var ba := 0.4 + hover_t * 0.4
		var bw := 1.2 + hover_t * 0.8
	...
	if sold:
		...
		# === 可购 / 不可购 ===
		var content_alpha := draw_alpha if can_afford else draw_alpha * 0.55
		...
		if desc_text != "":
			var desc_c := Color(t.info.r, t.info.g, t.info.b, content_alpha * 0.78)
		...
		var price_alpha := content_alpha if can_afford else draw_alpha * 0.67
		draw_string(font, Vector2(0, hh - 8),
...
func _draw_card_icon(info: Dictionary, item_key: String, center: Vector2, alpha: float) -> void:
	...
	if tex:
		var icon_size := 36.0
		var tex_rect := Rect2(center.x - icon_size / 2.0, center.y - icon_size / 2.0, icon_size, icon_size)
...
func _draw_buttons() -> void:
	...
	# ── 刷新按钮 ──
	if _refresh_t > 0.01:
		var rect := _get_refresh_button_rect()
		var h_t := _hover_refresh_t
		...
		var s_btn := 1.0 + h_t * 0.05
		# 微缩放在按钮中心
		var btn_cx := rect.get_center().x
		var btn_cy := rect.get_center().y
		...
		var text_a := 0.92 if can_refresh else 0.4
	...
	# ── 离开按钮 ──
	if _leave_t > 0.01:
		var rect := _get_leave_button_rect()
		var h_t := _hover_leave_t
